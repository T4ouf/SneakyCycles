...
func generateColorSwap(ref_image_path:String, color_swap_palette_path:String, lightColor:Color, darkColor:Color) -> void :
	
	# Load the source image and the color palet 
	var image:Image = Image.load_from_file(ref_image_path)
	var color_swap_palette:Image = Image.load_from_file(color_swap_palette_path)
	
	# get the amount of new images to create
	var color_swap_num:int = color_swap_palette.get_width();
		...
		for pixel_y:int in range(image.get_height()):
			for pixel_x:int in range(image.get_width()):
				if(color_swap_image.get_pixel(pixel_x, pixel_y)  == lightColor) :
					color_swap_image.set_pixel(pixel_x, pixel_y, color_swap_palette.get_pixel(i, 0));
				elif(color_swap_image.get_pixel(pixel_x, pixel_y)  == darkColor) :
					color_swap_image.set_pixel(pixel_x, pixel_y, color_swap_palette.get_pixel(i, 1));
